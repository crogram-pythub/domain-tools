...
# REF: https://www.cnblogs.com/libitum/p/14848615.html

Example:

    from tkinter import Tk
    from tkAsyncEvent import AsyncEvent

    App = Tk()
    async_executor = AsyncEvent(App)
    ...
    async_executor.submit(run_in_thread, parameter).then(get_result)
'''
            ...
            if future.done():
                if future.exception():
                    if on_failure:
                        ...
                        raise future.exception() or BaseException()
                        # add log for the exception.
                        print('add log for the exception')
                elif on_success:
